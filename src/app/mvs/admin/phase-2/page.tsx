...
import { loadPhase2Snapshot } from '@/lib/dashboard';
...
export default async function AdminPhase2Page({ searchParams }: PageProps) {
  ...
  // Editor view needs scenario summary + delta charts; responses view
  // needs only the response payload. Both views need the badge count.
  const [scenario, snapshot, responses, responsesCount] = await Promise.all([
    view === 'editor' ? getScenarioByCode(scenarioCode) : Promise.resolve(null),
    view === 'editor' ? loadPhase2Snapshot() : Promise.resolve(null),
    view === 'responses'
      ? getResponsesByCodes([scenarioCode], 'post')
      : Promise.resolve([]),
    view === 'responses'
      ? Promise.resolve(0)
      : countResponsesByCodes([scenarioCode], 'post'),
  ]);
          ...
          <>
            <section className="bg-white border border-zinc-200 rounded-xl p-4">
              ...
              <p className="text-sm text-zinc-500 mb-4">
                Post completion: {snapshot?.postCompletion?.completed ?? 0} of{' '}
                {snapshot?.postCompletion?.enrolled ?? 0} students.
              </p>
              <Phase1To2Delta
